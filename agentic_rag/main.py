...
from config import api_key, base_url, model_llm_embeddings, model_llm_responses, collection_name, collection_name_active, retrieval_method
...
def main():
    ...
    if create_bd:
        #cargar datos
        df = pd.read_excel('./data/input/calificaciones_completa.xlsx')
        ...
        for file_name in os.listdir(output_folder):
            ...
            if os.path.isfile(file_path):
                ...
                store_document_embeddings(text, chunk_id,  client, collection_name, model_llm_embeddings, chunk_size, overlap_size)
        ...
        if use_agents:
            ...
            question = "¿Cuál es la edad, sexo y correo del estudiante llamado beatriz juana blanco sanz?"
            #question = "¿Cómo se comparan las notas del bimestre 1 y bimestre 2 para el estudiante con identificador estudiante: 87812?"
            
            response = query_chroma_with_llm(client, question, collection_name_active, model_llm_embeddings, model_llm_responses, retrieval_method)
            print(response)
